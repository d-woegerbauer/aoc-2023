...
input.forEach(line => {
  ...
  differences.reverse().forEach(difference => {
    if(differenceColumn.length == 0){
      differenceColumn.push(0);
      return;
    }
    differenceColumn.push(difference[0]-differenceColumn[differenceColumn.length-1]);
  });

  sum += differenceColumn[differenceColumn.length-1];
});
